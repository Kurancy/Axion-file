...
import { MapPin, Globe, Activity, ShieldCheck, RefreshCw } from "lucide-react";
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col items-center">
          <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 w-full border-b border-slate-800/20 pb-4">
            <div className="text-center sm:text-left">
              <h4 className={`text-xs font-bold tracking-widest uppercase font-mono flex items-center justify-center sm:justify-start gap-2 ${
                isDarkMode ? "text-brand-blue-light" : "text-brand-blue"
              }`}>
                <Globe className="w-4 h-4 animate-spin-slow text-brand-blue-light" />
                Sovereign Grid Architecture
              </h4>
              <p className={`text-xs mt-1 ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                Interactive telemetry mapping Axion Core Integration nodes.
              </p>
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className={`px-3 py-1.5 rounded-lg border text-xs font-mono font-bold flex items-center justify-center gap-2 transition-all cursor-pointer select-none self-center sm:self-auto ${
                isDarkMode
                  ? "hover:bg-slate-800 border-slate-800 text-slate-300 disabled:opacity-50"
                  : "hover:bg-slate-100 border-slate-200 text-slate-700 disabled:opacity-50 shadow-sm"
              }`}
              title="Sync Mesh Telemetry"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? "animate-spin text-brand-gold" : ""}`} />
              {isLoading ? "Syncing..." : "Sync Mesh"}
            </button>
          </div>
          ...
          <div className="relative w-full max-w-[360px] h-[380px] flex items-center justify-center">
            ...
            >
              ...
              <path
                ...
                className={`transition-colors duration-300 fill-current opacity-20 ${isLoading ? "animate-pulse" : ""}`}
              />
            </svg>

            {isLoading ? (
              <div id="map-telemetry-scanning-overlay" className="absolute inset-0 flex flex-col items-center justify-center rounded-xl pointer-events-none">
                <div className="absolute top-1/2 -translate-y-1/2 flex flex-col items-center gap-3">
                  <div className="w-14 h-14 rounded-full border-2 border-brand-blue-light/30 border-t-brand-blue-light animate-spin flex items-center justify-center">
                    <Activity className="w-5 h-5 text-brand-gold animate-pulse" />
                  </div>
                  <span className="font-mono text-[10px] tracking-widest text-brand-blue-light font-bold uppercase text-center bg-slate-950/80 px-2.5 py-1 rounded-md border border-slate-800">
                    Triangulating Core Grid...
                  </span>
                </div>
                
                {/* Horizontal scanner light bar sweep */}
        ...
        <div id="diagnostics-panel" className="lg:col-span-5 h-full">
          ...
          >
            {isLoading ? (
              <div id="diagnostics-skeleton" className="space-y-6">
                <div className="flex justify-between items-start border-b border-slate-800/50 pb-4">
                  <div className="space-y-2 flex-1">
                    <div className={`h-5 w-2/3 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                    <div className={`h-3 w-1/2 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                  </div>
                  <div className={`h-4.5 w-14 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                </div>

                <div className="space-y-4">
                  {[1, 2].map((i) => (
                    <div key={i} className="space-y-1.5">
                      <div className={`h-2.5 w-1/3 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                      <div className={`h-4 w-1/2 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                    </div>
                  ))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[1, 2].map((i) => (
                      <div key={i} className="space-y-1.5">
                        <div className={`h-2.5 w-2/3 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                        <div className={`h-4 w-1/2 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                      </div>
                    ))}
                  </div>
                ...
                <div className="border-t border-slate-800/50 pt-4 flex justify-between items-center">
                  <div className={`h-3.5 w-1/3 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                ...
                <div className="border-t border-slate-800/50 pt-4 space-y-2">
                  <div className={`h-2.5 w-1/4 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                  <div className="flex flex-wrap gap-1.5">
                    {[1, 2, 3, 4, 5].map((idx) => (
                      <div key={idx} className={`h-6 w-12 rounded ${isDarkMode ? "shimmer-dark" : "shimmer-light"}`} />
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {hubsData.map((hub) => (
                      <button
                        ...
                        disabled={isLoading}
                        onClick={() => setSelectedHub(hub)}
                        className={`px-2.5 py-1 text-xs rounded transition-all select-none cursor-pointer ${
                          selectedHub.id === hub.id
                            ? "bg-brand-blue text-white shadow font-semibold"
                            : isDarkMode
                            ? "bg-slate-900 hover:bg-slate-800 text-slate-400"
                            : "bg-slate-100 hover:bg-slate-200 text-slate-600 shadow-sm"
                        }`}
                      >
